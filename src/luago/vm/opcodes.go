...
/* OpArgMask */
const (
	OpArgN = iota // argument is not used 参不需要使用
	OpArgU        // argument is used	  参数会被使用
	OpArgR        // argument is a register or a jump offset		参数是一个寄存器或者是一个跳跃偏移
	OpArgK        // argument is a constant or register/constant	参数是一个常量或者寄存器/常量
)
...
//注意,这个指令实际上比Lua原生的多了两个块
type opcode struct{
	testFlag byte	//定义操作符是否是一个测试(下一个指令必然是jump)
	setAFlag byte   //设置一个寄存器A
	argBMode byte   //B参数模式
	argCMode byte   //C参数模式
	opMode   byte   //op模式
	name     string //指令名字
